...
- **汇总 CSV**：`test/xlinkers_output.csv`（含 13 个常见 + 4 个已收录测试交联剂 + 本次输入，utf-8-sig）
...
![CrosslinkerProps 测试结果比较图](README/xlinkers_comparison.png)
...
| sulfo-SDA             | 光（磺化）         | 纯化蛋白（膜不可渗透，无细胞原位报道）                             | [Müller et al., _Anal. Chem._ 2019, 91:9041, 10.1021/acs.analchem.9b01339](https://doi.org/10.1021/acs.analchem.9b01339)（sulfo-SDA 交联 HSA / 细胞色素 c，纯化蛋白）；[Belsom et al., _Wellcome Open Res._ 2016, 1:24, 10.12688/wellcomeopenres.10129.1](https://doi.org/10.12688/wellcomeopenres.10129.1)（CASP11 盲测，纯化蛋白靶标）；[Pompach et al., _Front. Endocrinol._ 2019, 10:695, 10.3389/fendo.2019.00695](https://doi.org/10.3389/fendo.2019.00695)（IGF-1:Imp-L2 纯化复合物） |
| LC-SDA                | 光（长链）         | 待补充（无蛋白层级实验文献；制造商声明膜可渗透，适用于细胞内交联） | [Khan et al., _Nanomaterials_ 2022, 12:608, 10.3390/nano12040608](https://doi.org/10.3390/nano12040608)（材料表面偶联应用，注意其使用的是磺化版本 sulfo-NHS-LC-diazirine / SNLD）；[Thermo 官方手册 _Amine-Reactive Diazirine Crosslinkers_ 2067.0](https://assets.thermofisher.com/TFS-Assets/LSG/manuals/2067_AmineReactDiazirineCrosslnk_UG.pdf)（间隔臂 12.5 Å，膜可渗透，用于细胞内/膜内偶联）；[Sigma-Aldrich 产品页](https://www.sigmaaldrich.cn/CN/zh/product/aldrich/803421)        |
...
> 2. LC-SDA（succinimidyl 6-(4,4′-azipentanamido)hexanoate，间隔臂 12.5 Å）为 SDA 的长链类似物，
>    其膜可渗透性来自 Thermo 官方手册（NHS 酯类 diazirine 均无带电基团、膜可渗透）；**PNAS 2017（Walker-Gray et al.）使用的是 SDA（短链 3.9 Å），不是 LC-SDA**。
>    检索未发现明确使用 LC-SDA 的蛋白交联应用文献（Khan et al. 2022 _Nanomaterials_ 用的是其磺化版本 sulfo-LC-SDA）。
> 3. sulfo-SDA 因含磺酸基为水溶性、膜不可渗透，文献中未见细胞原位（活细胞）应用。
> 4. DCDS 为本项目组自合成交联剂，尚无公开文献，引文与测试层级待补充。
